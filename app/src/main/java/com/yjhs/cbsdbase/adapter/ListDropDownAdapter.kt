 ...
 */
class ListDropDownAdapter(data: ArrayList<String>) :
    ...
    override fun convert(helper: BaseViewHolder?, item: String?) {
        ...
        if (mSelectPosition !== -1) {
            if (mSelectPosition === helper?.layoutPosition) {
                ...
                helper?.getView<TextView>(R.id.text).setBackgroundResource(R.color.check_bg)
            } else {
                helper?.getView<TextView>(R.id.text)?.isSelected = false
                helper?.getView<TextView>(R.id.text)?.setBackgroundResource(R.color.white)
            }
        }
    }

    fun setSelectPosition(selectPosition: Int): ListDropDownAdapter {
